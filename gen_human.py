...
for subject in train_list:
    ...
    for aid,action in enumerate( data3d_item[subject]):
        ...
        for frame, skeleton_t in enumerate(p3s):
            ...
            skeleton_1 =word2cam(p3s[frame+1])
            skeleton_2 =word2cam( p3s[frame+2])
            skeleton_3 =word2cam( p3s[frame+3])
            skeleton_4 =word2cam( p3s[frame+4])
            skeleton_5 =word2cam(p3s[frame+5])

            uv_1 = get12(cam2uv(skeleton_1))
            uv_2 = get12(cam2uv(skeleton_2))
            uv_3 = get12(cam2uv(skeleton_3))
            uv_4 = get12(cam2uv(skeleton_4))
            uv_5 = get12(cam2uv(skeleton_5))
